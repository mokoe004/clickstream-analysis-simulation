...
def generate_session_events(session_id: str, user_id: str, base_time: datetime) -> List[Dict]:
    events = []
    steps = random.choices([2, 3, 4, 5], weights=[0.1, 0.2, 0.4, 0.3])[0]
...
def send_events_to_kafka(events: List[Dict]):
    ...
    for event in events:
        ...
        print("Event has been sent to kafka.")
        time.sleep(0.05)  # Begrenzung optional
...
# Hauptfunktion
# ----------------------------
def generate_clickstream_data(days: int = 90, sessions_per_day: int = 100,
                              output_format: str = "json", send_to_kafka: bool = False):
    start_date = datetime.utcnow() - timedelta(days=days)

    for day_offset in range(days):
        current_day = start_date + timedelta(days=day_offset)
        date_str = current_day.strftime('%Y-%m-%d')
        filename = OUTPUT_DIR / f"clickstream_{date_str}.{output_format}"
        ...
        for _ in range(sessions_per_day):
            ...
            daily_events.extend(generate_session_events(session_id, user_id, session_time))

        if output_format == "json":
            save_events_to_json(daily_events, filename)
        elif output_format == "csv":
            save_events_to_csv(daily_events, filename)

        if send_to_kafka:
            send_events_to_kafka(daily_events)

        print(f"✅ {len(daily_events)} Events für {date_str} gespeichert ({output_format})")
